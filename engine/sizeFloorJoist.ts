...
function loadTable(liveLoadPsf: number): any {
  // Validate live load
  if (liveLoadPsf !== 30 && liveLoadPsf !== 40) {
    throw new Error('Live load must be 30 or 40 psf');
  }
  const tableName = liveLoadPsf === 30 ? 'R502.3.1(1)' : 'R502.3.1(2)';
...
export function sizeFloorJoist(
  ...
  // Live load validation is now done in loadTable, but we can also do it here for consistency
  if (liveLoadPsf !== 30 && liveLoadPsf !== 40) {
    return {
      ok: false,
      reason: 'Live load must be 30 or 40 psf',
      detail: [`Provided live load: ${liveLoadPsf} psf`]
    };
  }

  try {
    const table = loadTable(liveLoadPsf);
